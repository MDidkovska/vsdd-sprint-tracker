/**
 * Pure leadership filtering + summary semantics. Lives in the domain layer so
 * BOTH the Leadership View and the export path use identical filtering (review
 * finding: filtered export must match the on-screen population exactly).
 */
import type {
  LeadershipFilters,
  LeadershipSnapshot,
...
export function matchesFilters(cell: LeadershipTeamCell, filters: LeadershipFilters): boolean {
  ...
  if (filters.rag !== 'ALL') {
    ...
    if (!rag) return false;
    if (rag.business !== filters.rag && rag.delivery !== filters.rag && rag.release !== filters.rag) {
      return false;
    }
  }
  return true;
